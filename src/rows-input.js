...
export default class TableInput extends React.Component {
  ...
  updateCell = (e, rowIndex, cellIndex) => {
    ...
    newValue[rowIndex].cells[cellIndex] = e.target.value;
  ...
  initializeTable = () => {
    ...
    const newValue = [{ _type: 'tableRow', _key: uuid(), cells: [''] }];
    return onChange(createPatchFrom(newValue));
  };
  ...
  addRow = e => {
    ...
    const columnCount = value[0].cells.length;
